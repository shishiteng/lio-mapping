                                   ...
                                   const std_msgs::Header &header)
{
  ...
  DLOG(INFO) << endl
             << "transform_aft_mapped_[" << header.stamp.toSec() << "]: " << transform_aft_mapped_;
...
bool Estimator::RunInitialization()
{
  ...
  bool init_result = ImuInitializer::Initialization(all_laser_transforms_, Vs_, Bas_, Bgs_, g_vec_in_laser, transform_lb_, R_WI_);
  //  init_result = false;
  ...
  for (int i = 0; i < corner_points_size; i++)
  {
    ...
    if (point_search_sq_dis[4] < min_match_sq_dis_)
    {
      ...
      if (mat_D1(0, 2) > 3 * mat_D1(0, 1))
      {
        ...
        float ld_p1 = -(normal_to_point.x() * point_proj.x + normal_to_point.y() * point_proj.y + normal_to_point.z() * point_proj.z);
        float ld_p2 = -(normal_cross_point.x() * point_proj.x + normal_cross_point.y() * point_proj.y + normal_cross_point.z() * point_proj.z);
                                   ...
                                   vector<unique_ptr<Feature>> &features)
{
  ...
  for (size_t iter_count = 0; iter_count < num_max_iterations_; ++iter_count)
  {
    ...
    if (!isfinite(local_transform.pos.x()))
      local_transform.pos.x() = 0.0;
    if (!isfinite(local_transform.pos.y()))
      local_transform.pos.y() = 0.0;
    if (!isfinite(local_transform.pos.z()))
      local_transform.pos.z() = 0.0;
...
void Estimator::SolveOptimization()
{
  ...
  if (estimator_config_.update_laser_imu)
  {
    ...
    {
      Twist<double>
          incre = (transform_lb_.inverse() * all_laser_transforms_[cir_buf_count_ - 1].second.transform.inverse() * all_laser_transforms_[cir_buf_count_].second.transform * transform_lb_).cast<double>();
